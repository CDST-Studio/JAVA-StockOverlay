...
if type(json_data) == type([]) :
    for dic in json_data :
        ...
        if len(int_stockCode) < 6 :
            ...
            dic_data[dic['name']] = stockCode
        # 그 외의 경우
        else :
            dic_data[dic['name']] = int_stockCode
...
# 파이어베이스 DB 정보 가져오기
db = firestore.client()
doc_ref = db.collection(u'Stock').document(u'StockNameToCode') # collection(u'(FireStore의 컬렉션 이름)').document(u'(FireStore의 특정 컬렉션 안의 문서 이름)')
doc_ref.set(dic_data) ; # dictionary 형태인 KEY : VALUE 형식으로 FireStore에 저장된다.